...
def ExtractMultipleImages():
        ...
        for file in files:
                ...
                for n in range(0, round(tif.shape[0] / sample)):
                        tifffile.imwrite(
                                "C:/Users/binh_nguyen/Documents/test_folder_1/test_ipc_log_data/domain_image/splitted_image/" + os.path.basename(file)[:-4] + str(file_num) + "_clone_%s.png" % str(n),
                                tif[sample * n:sample * (n + 1)])
                        count = n
                tifffile.imwrite(
                        "C:/Users/binh_nguyen/Documents/test_folder_1/test_ipc_log_data/domain_image/splitted_image/" + os.path.basename(file)[:-4] + str(file_num) + "_clone_%s.png" % str(count + 1),
        ...
        for file in files:
                ...
                for n in range(0, round(tif.shape[0] / sample)):
                        for m in range(0, int(tif.shape[1] / sample)):
                                ...
                                tifffile.imwrite(
                                        "C:/Users/binh_nguyen/Documents/test_folder_1/test_ipc_log_data/domain_image/processing_piece/" + os.path.basename(file)[:-4] + "_clone_%s_%s.png" % (
                                        str(n), str(m)),
